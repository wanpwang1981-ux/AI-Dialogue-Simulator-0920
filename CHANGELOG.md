...
### 新增 (Added)
- **國際化 (i18n) 支援**:
    - 引入完整的國際化框架 (`gettext`, `Babel`)，支援多語言介面。
    - 提供英文 (`en`) 與繁體中文 (`zh_TW`) 的完整介面翻譯。
    - 在「設定」選單中新增語言切換功能。
    - 新增 `compile_translations.sh` 腳本以簡化翻譯檔案的更新與編譯流程。
- **`.env` 環境變數整合**:
    - API金鑰管理重構，現在從 `.env` 檔案讀取 `GEMINI_API_KEY`，取代原有的 `config.json` 儲存方式，提升安全性。
    - 移除了UI中的「API金鑰管理」視窗。
- **新增 VIBER Meta-Prompts**:
    - 新增 `VIBER.MD` 檔案，其中包含一組整合性的元提示詞(meta-prompts)，可用於指導AI進行更高品質、多面向的辯論。

### 變更 (Changed)
- **預設語言**: 應用程式的預設語言已變更為繁體中文 (`zh_TW`)。

### 安全性 (Security)
- **金鑰安全**: 將 `config.json` 與 `.env` 檔案新增至 `.gitignore`，防止敏感金鑰被意外提交。
